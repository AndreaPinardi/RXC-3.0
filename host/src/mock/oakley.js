window.config = {
  actionsModule: {
    isLoadingLenses: false,
    currentFrameId: 18079722,
    delegateLearnMoreContent: function (contentIdentifier) {
      window.alert("funzione chiamata");
    },
  },
  layoutSettings: {
    enableDigitalOptometry: true,
    enableLargeIcons: true,
  },
  baseURLs: {
    genericImage: "https://assets.oakley.com/extra/image/rxc/brands/oakley/",
    projectImage: "https://media.oakley.com/2021/Lens_Configurator/",
    framesImage: "https://assets.oakley.com/extra/image/rxc/frames/",
    assetsCDN: "https://assets.oakley.com",
  },
  brand: "oakley",
  currencyFormat: {
    thousandSeparator: ",",
    decimalSeparator: ".",
    prefix: "$",
  },
  data: {
    frame: {
      catEntryId: "888392326843_RX",
      name: "Frogskins™ Lite",
      upc: "888392326843",
      model: "0OO9374",
      brand: "Oakley",
      color: "937402",
      category: "SUNGLASSES",
      size: "M (132mm)",
      imageUrl:
        "https://assets2.oakley.com/cdn-record-files-pi/0f1407ab-fb12-489c-8d93-a802012ecfa3/c6bcaf11-5297-4474-bcc1-ae530116d5c1/0OO9374__937402__P21__shad__qt.png?impolicy=OO_ratio&width=1000",
      rxValues: {
        powerCombinedMin: "-5.0",
        powerCombinedMax: "4.0",
      },
      listPrice: "19470.0",
      offerPrice: "19470.0",
    },
    vatIncluded: true,
    frameOnlyLensUPC: "99",
    showFrameOnly: false,
  },
  lensesData: {
    packages: [
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001622361",
          productUPC: "Prizm_Trail",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Trail",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Trail_APAC_SV",
          lensClass: "2N",
          polar: null,
          colorCategory: "PRIZM_SPORT",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: ["マウンテンバイク"],
          designType: "PrizmSport",
          bundleName: null,
          clen: "8280O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ スポーツ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Trail",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623634",
          productUPC: "Prizm_Trail_Torch",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Trail Torch",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Trail_Torch_APAC_SV",
          lensClass: "2N",
          polar: null,
          colorCategory: "PRIZM_SPORT",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: ["マウンテンバイク"],
          designType: "PrizmSport",
          bundleName: null,
          clen: "8967O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "6.0",
            },
            cyl: {
              min: "-7.0",
              max: "7.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ スポーツ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Trail Torch",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001622347",
          productUPC: "Prizm_Golf",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Golf",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Golf_APAC_SV",
          lensClass: "2N",
          polar: null,
          colorCategory: "PRIZM_SPORT",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: ["ゴルフ"],
          designType: "PrizmSport",
          bundleName: null,
          clen: "8273O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "6.0",
            },
            cyl: {
              min: "-7.0",
              max: "7.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ スポーツ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Golf",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623566",
          productUPC: "Prizm_Grey_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Grey Polarized",
          listPrice: 51700,
          offerPrice: 39700,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Grey_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "PRIZM_POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8920",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "6.0",
            },
            cyl: {
              min: "-7.0",
              max: "7.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥71,170",
            totalOfferPriceFormatted: "¥59,170",
            totalListPrice: 71170,
            totalOfferPrice: 59170,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "Prizm Grey Polarized",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623405",
          productUPC: "Prizm_Deep_Water_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Deep Water Polarized",
          listPrice: 53900,
          offerPrice: 43700,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Deep_Water_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "PRIZM_SPORT",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: ["釣り"],
          designType: "PrizmSport",
          bundleName: null,
          clen: "8281O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "6.0",
            },
            cyl: {
              min: "-7.0",
              max: "7.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥73,370",
            totalOfferPriceFormatted: "¥63,170",
            totalListPrice: 73370,
            totalOfferPrice: 63170,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ スポーツ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Deep Water Polarized",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623627",
          productUPC: "Prizm_Road_Jade",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Road Jade",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Road_Jade_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM_SPORT",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: ["サイクリング", "トレーニング/ランニング"],
          designType: "PrizmSport",
          bundleName: null,
          clen: "8963O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ スポーツ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Road Jade",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623610",
          productUPC: "Prizm_Road_Black",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Road Black",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Road_Black_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM_SPORT",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: ["サイクリング", "トレーニング/ランニング"],
          designType: "PrizmSport",
          bundleName: null,
          clen: "8962O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ スポーツ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Road Black",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623580",
          productUPC: "Prizm_24K_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm 24K Polarized",
          listPrice: 53900,
          offerPrice: 43700,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_24K_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "PRIZM_POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8939O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥73,370",
            totalOfferPriceFormatted: "¥63,170",
            totalListPrice: 73370,
            totalOfferPrice: 63170,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ 24K Polarized",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623443",
          productUPC: "Prizm_Sapphire_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Sapphire Polarized",
          listPrice: 53900,
          offerPrice: 43700,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Sapphire_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "PRIZM_POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8922O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "6.0",
            },
            cyl: {
              min: "-7.0",
              max: "7.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥73,370",
            totalOfferPriceFormatted: "¥63,170",
            totalListPrice: 73370,
            totalOfferPrice: 63170,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Sapphire Polarized",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001622378",
          productUPC: "Prizm_Field",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Field",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Field_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM_SPORT",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: ["野球"],
          designType: "PrizmSport",
          bundleName: null,
          clen: "2275",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ スポーツ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Field",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623429",
          productUPC: "Prizm_Jade_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Jade Polarized",
          listPrice: 53900,
          offerPrice: 43700,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Jade_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "PRIZM_POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8923O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "6.0",
            },
            cyl: {
              min: "-7.0",
              max: "7.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥73,370",
            totalOfferPriceFormatted: "¥63,170",
            totalListPrice: 73370,
            totalOfferPrice: 63170,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Jade Polarized",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001622354",
          productUPC: "Prizm_Road",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Road",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Road_APAC_SV",
          lensClass: "2N",
          polar: null,
          colorCategory: "PRIZM_SPORT",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: ["サイクリング", "トレーニング/ランニング"],
          designType: "PrizmSport",
          bundleName: null,
          clen: "8279O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ スポーツ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Road",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623436",
          productUPC: "Prizm_Ruby_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Ruby Polarized",
          listPrice: 53900,
          offerPrice: 43700,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Ruby_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "PRIZM_POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8931O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥73,370",
            totalOfferPriceFormatted: "¥63,170",
            totalListPrice: 73370,
            totalOfferPrice: 63170,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Ruby Polarized",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623603",
          productUPC: "Prizm_Rose_Gold_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Rose Gold Polarized",
          listPrice: 53900,
          offerPrice: 43700,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Rose_Gold_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "PRIZM_POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8958O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥73,370",
            totalOfferPriceFormatted: "¥63,170",
            totalListPrice: 73370,
            totalOfferPrice: 63170,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Rose Gold Polarized",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623412",
          productUPC: "Prizm_Black_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Black Polarized",
          listPrice: 53900,
          offerPrice: 43700,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Black_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "PRIZM_POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8921O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "6.0",
            },
            cyl: {
              min: "-7.0",
              max: "7.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥73,370",
            totalOfferPriceFormatted: "¥63,170",
            totalListPrice: 73370,
            totalOfferPrice: 63170,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Black Polarized",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623184",
          productUPC: "Positive_Red_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Positive Red Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Positive_Red_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2019",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "Positive Red Iridium",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001622385",
          productUPC: "Emerald_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Emerald Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Emerald_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2158",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "エメラルドイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623160",
          productUPC: "Tungsten_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Tungsten Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Tungsten_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "1989",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "タングステンイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623115",
          productUPC: "Jade_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Jade Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Jade_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2016",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ジェイドイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623108",
          productUPC: "Ice_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Ice Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Ice_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2164",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "アイスイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623153",
          productUPC: "Torch_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Torch Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Torch_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "8889",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "トーチイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623191",
          productUPC: "Grey",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Grey",
          listPrice: 37400,
          offerPrice: 26500,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Grey_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "1893",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥56,870",
            totalOfferPriceFormatted: "¥45,970",
            totalListPrice: 56870,
            totalOfferPrice: 45970,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "グレー",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623146",
          productUPC: "Sapphire_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Sapphire Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Sapphire_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2229",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "サファイアイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623122",
          productUPC: "Ruby_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Ruby Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Ruby_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2018",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ルビーイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623177",
          productUPC: "Violet_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Violet Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Violet_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2017",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "バイオレットイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001622330",
          productUPC: "Black_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Black Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: true,
          antiReflective: "Premium",
          upc: "Black_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "1957",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ブラックイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001622392",
          productUPC: "Fire_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Fire Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Fire_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2156",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ファイアーイリジウム",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623139",
          productUPC: "24K_Iridium",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "24K Iridium",
          listPrice: 39600,
          offerPrice: 27600,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "24K_Iridium_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "STANDARD",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2106",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-6.0",
              max: "8.0",
            },
            cyl: {
              min: "-5.0",
              max: "5.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥59,070",
            totalOfferPriceFormatted: "¥47,070",
            totalListPrice: 59070,
            totalOfferPrice: 47070,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "24K Iridium",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623306",
          productUPC: "Sapphire_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Sapphire Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Sapphire_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2232",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "サファイアイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623221",
          productUPC: "Bronze_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Bronze Polarized",
          listPrice: 46200,
          offerPrice: 35300,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Bronze_Polarized_APAC_SV",
          lensClass: "2P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "1592",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥65,670",
            totalOfferPriceFormatted: "¥54,770",
            totalListPrice: 65670,
            totalOfferPrice: 54770,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ブロンズ偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623252",
          productUPC: "Fire_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Fire Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Fire_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2154",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ファイアーイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623238",
          productUPC: "Deep_Blue_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Deep Blue Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Deep_Blue_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "8249",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ディープブルーイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623269",
          productUPC: "Grey_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Grey Polarized",
          listPrice: 46200,
          offerPrice: 35300,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Grey_Polarized_APAC_SV",
          lensClass: "2P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "1669",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥65,670",
            totalOfferPriceFormatted: "¥54,770",
            totalListPrice: 65670,
            totalOfferPrice: 54770,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "グレー偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623320",
          productUPC: "Torch_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Torch Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Torch_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "8891",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "トーチイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623207",
          productUPC: "24K_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "24K Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "24K_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2105",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "24K Iridium Polarized",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623245",
          productUPC: "Emerald_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Emerald Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Emerald_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2025",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "エメラルドイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623313",
          productUPC: "Shallow_Blue_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Shallow Blue Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Shallow_Blue_Iridium_Polarized_APAC_SV",
          lensClass: "2P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2166",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "シャローブルーイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623337",
          productUPC: "Tungsten_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Tungsten Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Tungsten_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2026",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "タングステンイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623290",
          productUPC: "Ruby_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Ruby Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Ruby_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2145",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ルビーイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623283",
          productUPC: "Jade_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Jade Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Jade_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2167",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ジェイドイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623344",
          productUPC: "Violet_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Violet Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Violet_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2147",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "バイオレットイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623276",
          productUPC: "Ice_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Ice Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Ice_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2165",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "アイスイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623214",
          productUPC: "Black_Iridium_Polarized",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Black Iridium Polarized",
          listPrice: 48400,
          offerPrice: 36400,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Black_Iridium_Polarized_APAC_SV",
          lensClass: "3P",
          polar: "polar-true",
          colorCategory: "POLARIZED",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "OakleyStandard",
          bundleName: null,
          clen: "2023",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-4.0",
              max: "6.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥67,870",
            totalOfferPriceFormatted: "¥55,870",
            totalListPrice: 67870,
            totalOfferPrice: 55870,
            markdownPercentage: null,
            polarTitle: "ポラライズド（偏光）",
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "スタンダード",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "ブラックイリジウム 偏光レンズ",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623528",
          productUPC: "Prizm_Jade",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Jade",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Jade_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8932O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Jade",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623597",
          productUPC: "Prizm_Rose_Gold",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Rose Gold",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Rose_Gold_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8957O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Rose Gold",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623542",
          productUPC: "Prizm_Sapphire",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Sapphire",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Sapphire_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8928O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Sapphire",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623573",
          productUPC: "Prizm_24K",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm 24K",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_24K_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8938O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ 24K",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623467",
          productUPC: "Prizm_Black",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Black",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Black_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8924O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Black",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623535",
          productUPC: "Prizm_Ruby",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Ruby",
          listPrice: 45100,
          offerPrice: 45100,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: false,
          antiReflective: "Premium",
          upc: "Prizm_Ruby_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8930O",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥64,570",
            totalOfferPriceFormatted: "¥64,570",
            totalListPrice: 64570,
            totalOfferPrice: 64570,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "PRIZM™ Ruby",
            markdownFixed: null,
          },
        },
      },
      {
        frame: {
          listPrice: "19470.0",
          offerPrice: "19470.0",
          insPrice: null,
          promoType: null,
          promo: null,
        },
        lensPackage: {
          blueLight: null,
          catEntryId: "20500001623511",
          productUPC: "Prizm_Grey",
          transition: null,
          type: "SINGLE_VISION",
          uvProtection: true,
          productCatEntryId: null,
          antiScratch: true,
          color: "Prizm Grey",
          listPrice: 42900,
          offerPrice: 42900,
          insPrice: null,
          insurable: true,
          brand: "Oakley",
          thickness: "Standard Thin",
          isBaseLens: true,
          antiReflective: "Premium",
          upc: "Prizm_Grey_APAC_SV",
          lensClass: "3N",
          polar: null,
          colorCategory: "PRIZM",
          lensMaterial: null,
          suitableForDriving: false,
          recommendedUse: null,
          designType: "PrizmEveryday",
          bundleName: null,
          clen: "8927",
          treatmentFamily: null,
          rxValues: {
            sph: {
              min: "-3.0",
              max: "2.0",
            },
            cyl: {
              min: "-3.0",
              max: "3.0",
            },
            add: null,
            ax: null,
            pd: {
              min: "25.0",
              max: "80.0",
            },
          },
          promo: null,
          lensPackageDetails: {
            totalListPriceFormatted: "¥62,370",
            totalOfferPriceFormatted: "¥62,370",
            totalListPrice: 62370,
            totalOfferPrice: 62370,
            markdownPercentage: null,
            polarTitle: null,
            uvProtectionTitle: "UV Protection",
            antiScratchTitle: "Anti-scratch",
            typeTitle: "単焦点レンズ",
            designTypeTitle: "Prizm™ エブリデイ",
            thicknessTitle: "Plutonite® 1.59 シン",
            antiReflectiveTitle: "Oakley Stealth™ Pro",
            blueLightTitle: null,
            lensName: "Prizm Grey",
            markdownFixed: null,
          },
        },
      },
    ],
    content: {
      blueLight: {
        "Blue light treatment for Gaming": {
          imageFileName1: "rxc-icon-treatment_family-prizm™_gaming_logo_oo",
          imageFileName2: "rxc-icon-3d-treatment_family-prizm™_gaming_logo_oo",
          title: "Prizm™ ゲーミング",
          description:
            "Prizm Gaming™ 2.0レンズは色とコントラストを増強し、有機ELディスプレイやLEDスクリーンからのブルーライトをカットするように設計されています。 400～500nmのLEDブルーライトを30%削減。",
        },
        "Premium Blu IQ": {
          imageFileName1: "rxc-icon-treatment_family-blue_light_filtering_oo",
          imageFileName2:
            "rxc-icon-3d-treatment_family-blue_light_filtering_oo",
          title: "オークリー ブルー レディー",
          description:
            "Oakley Blue Ready（ブルーレディ）は、ブルーバイオレットライトに対する日常的なフィルター性能を備えたデザインです。",
        },
        AntiBlue: {
          imageFileName1: "rxc-icon-treatment_family-blue_light_filtering_oo",
          imageFileName2:
            "rxc-icon-3d-treatment_family-blue_light_filtering_oo",
          title: "Oakley Blue Ready",
          description:
            "Oakley Blue Ready（ブルーレディ）は、ブルーバイオレットライトに対する日常的なフィルター性能を備えたデザインです。",
        },
      },
      color: {
        Grey_Polarized: {
          title: "グレー ポラライズド（偏光）",
          description:
            "さまざまな厳しい条件の中で、妥協のないプロテクションを提供します。",
          lensClass: 2,
          imageFileName1: "puck/Grey_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.50_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_1.67_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ESS_1.59_Standard": {
          title: "Clear",
        },
        "Clear_OTD_ADV_Plus_1.50_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Prizm Ruby": {
          title: "Prizm‐プリズム™ ルビー",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Ruby_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        "Clear_ESS_1.50_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ADV_1.67_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ADV_Plus_1.50_Standard": {
          title: "Clear",
        },
        Ruby_Iridium_Polarized: {
          title: "ルビー イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Ruby_Iridium_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm Jade Iridium": {
          title: "Prizm‐プリズム™ ジェードイリジウム",
          imageFileName1: "v5/pucks/Prizm_Jade_SV_puckrx",
        },
        Fire_Iridium_Polarized: {
          title: "ファイヤー イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Fire_Iridium_Polarized_SV_puckrx",
          lightTransmission: 16,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Prizm Jade": {
          title: "Prizm‐プリズム™ ジェード",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Jade_SV_puckrx",
          lightTransmission: 14,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.67_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_1.50_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_1.67_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_1.50_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_Plus_1.67_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_1.50_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Sapphire Iridium": {
          title: "サファイアイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Sapphire_Iridium_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_Plus_1.50_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        Tungsten_Iridium_Polarized: {
          title: "タングステン イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Tungsten_Iridium_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        Violet_Iridium_Polarized: {
          title: "バイオレット イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Violet_Iridium_Polarized_SV_puckrx",
          lightTransmission: 14,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_ESS_1.67_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ADV_1.50_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        Brown_Transitions: {
          title: "ブラウン Transitions® 1.59",
          description:
            "さまざまな照明条件に対応する、より薄いレンズが必要な場合に最適です。",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Prizm Jade Iridium Polarized": {
          title: "Prizm‐プリズム™ ジェードイリジウム",
          imageFileName1: "v5/pucks/Prizm_Jade_Polarized_SV_puckrx",
        },
        "Clear_OTD_ADV_1.50_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Prizm Road Jade Iridium": {
          title: "Prizm™ Road Jade Iridium",
          imageFileName1: "v5/pucks/Prizm_Road_Jade_SV_puckrx",
        },
        "Clear_OTD_ADV_1.50_Standard": {
          title: "Clear",
        },
        "Fire Iridium": {
          title: "ファイヤーイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Fire_Iridium_SV_puckrx",
          lightTransmission: 16,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Emerald Transitions": {
          title: "エメラルド Transitions®",
          imageFileName1: "v5/pucks/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ADV_1.50_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Emerald Iridium Polarized": {
          title: "エメラルドイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Emerald_Iridium_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm Road Black": {
          title: "Prizm™ ロード ブラック",
          description:
            "路面のわずかな変化を見つけ、石やくぼみなどの危険を素早く察知して、確実にパフォーマンスを発揮できるように設計されています。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Road_Black_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Torch Iridium Polarized": {
          title: "トーチイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Torch_Iridium_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        Prizm_Black_Polarized: {
          title: "Prizm™ ブラック ポラライズド（偏光）",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Black_Polarized_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_1.50_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_ESS_1.50_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Prizm Black": {
          title: "Prizm™ ブラック",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Black_SV_puckrx",
          sequence: 3,
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        Ruby_Iridium: {
          title: "ルビー イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Ruby_Iridium_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_ESS_1.50_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_1.59_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        Black_Iridium_Shield: {
          title: "ブラック イリジウム シールド",
          description:
            "2枚の個別の度付きレンズをシールドに装着することで、視力を補正します。",
          lensClass: 3,
          imageFileName1: "puck/Black_Iridium_Shield_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm Field Iridium": {
          title: "Prizm™ フィールド イリジウム",
          imageFileName1: "v5/pucks/Prizm_Field_SV_puckrx",
        },
        "Clear_OTD_ADV_1.50_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        "Prizm Tungsten": {
          title: "Prizm‐プリズム™ タングステン",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          imageFileName1: "v5/pucks/Prizm_Tungsten_SV_puckrx",
        },
        "Clear_OTD_1.67_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ADV_1.50_Premium": {
          title: "Clear",
        },
        "Clear_OTD_ESS_1.50_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_1.59_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        Sapphire: {
          title: "サファイア Transitions®",
          imageFileName1: "v5/pucks/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Grey_Transitions_1-50": {
          title: "グレー Transitions® 1.59",
          description:
            "さまざまな照明条件に対応する、より薄いレンズが必要な場合に最適です。",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_1.59_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Amethyst Transitions": {
          title: "アメジスト Transitions®",
          imageFileName1: "v5/pucks/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_1.59_Premium": {
          title: "Clear",
        },
        "Fire Iridium Polarized": {
          title: "ファイヤーイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Fire_Iridium_Polarized_SV_puckrx",
          lightTransmission: 16,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_1.50_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_ADV_1.59_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ADV_Plus_1.50_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ESS_1.67_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        Prizm_Grey_Polarized: {
          title: "Prizm™ グレー ポラライズド（偏光）",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Grey_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        Prizm_Sapphire: {
          title: "Prizm™ サファイア",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Sapphire_SV_puckrx",
          lightTransmission: 12,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        Torch_Iridium_Polarized: {
          title: "トーチ イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Torch_Iridium_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ESS_1.67_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Brown_Transitions_1-50": {
          title: "ブラウン Transitions®",
          description:
            "さまざまな照明条件に対応するレンズが必要な場合に最適です。",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ADV_Plus_1.59_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_ESS_1.67_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ESS_1.59_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        Tungsten_Iridium: {
          title: "タングステン イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Tungsten_Iridium_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_ESS_1.59_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_1.50_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Black Iridium": {
          title: "ブラックイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Black_Iridium_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_Plus_1.59_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_ADV_Plus_1.50_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ESS_1.50_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_1.50_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Tungsten Iridium Polarized": {
          title: "タングステンイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Tungsten_Iridium_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Violet Iridium Polarized": {
          title: "バイオレットイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Violet_Iridium_Polarized_SV_puckrx",
          lightTransmission: 14,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        Prizm_Golf: {
          title: "Prizm™ ゴルフ",
          description:
            "フェアウェイ、フリンジ、ラフの間の変化を見分け、距離が正確に測れるように設計されているため、コースが見極めやすくなります。",
          lensClass: 2,
          imageFileName1: "puck/Prizm_Golf_SV_puckrx",
          lightTransmission: 30,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Clear_OTD_1.67_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Prizm 24K Polarized": {
          title: "Prizm‐プリズム™ 24K",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_24K_Polarized_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        Black_Iridium_Polarized: {
          title: "ブラック イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Black_Iridium_Polarized_SV_puckrx",
          lightTransmission: 9,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        Grey_Transitions: {
          title: "グレー Transitions® 1.59",
          description:
            "さまざまな照明条件に対応するレンズが必要な場合に最適です。",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_ESS_1.67_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "24K_Iridium": {
          title: "24K イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/24K_Iridium_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_ADV_1.67_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Torch Iridium": {
          title: "トーチイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Torch_Iridium_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm 24K Iridium": {
          title: "Prizm‐プリズム™ 24Kイリジウム",
          imageFileName1: "v5/pucks/Prizm_24K_SV_puckrx",
        },
        "Clear_OTD_ADV_Plus_1.50_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Shallow Blue Iridium Polarized": {
          title: "シャローブルーイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 2,
          imageFileName1: "v5/pucks/Shallow_Blue_Iridium_Polarized_SV_puckrx",
          lightTransmission: 21,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_ESS_1.50_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_1.67_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_ADV_Plus_1.67_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_ESS_1.50_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Prizm Golf Iridium": {
          title: "Prizm™ Golf Iridium",
          imageFileName1: "v5/pucks/Prizm_Golf_SV_puckrx",
        },
        "Brown Transitions": {
          title: "ブラウン Transitions®",
          description:
            "さまざまな照明条件に対応するレンズが必要な場合に最適です。",
          imageFileName1: "puck/brown-lens",
          imageFileName2: "puck/brown-lens",
        },
        "Prizm Golf": {
          title: "Prizm™ ゴルフ",
          description:
            "フェアウェイ、フリンジ、ラフの間の変化を見分け、距離が正確に測れるように設計されているため、コースが見極めやすくなります。",
          lensClass: 2,
          imageFileName1: "v5/pucks/Prizm_Golf_SV_puckrx",
          lightTransmission: 30,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        Amethyst: {
          title: "アメジスト Transitions®",
          imageFileName1: "v5/pucks/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_ESS_1.50_Standard": {
          title: "Clear",
        },
        "Clear_OTD_ADV_1.59_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        Jade_Iridium: {
          title: "ジェード イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Jade_Iridium_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm Grey Polarized": {
          title: "Prizm‐プリズム™ グレー",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Grey_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.59_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Grey Transitions": {
          title: "グレー Transitions®",
          description:
            "さまざまな照明条件に対応する、より薄いレンズが必要な場合に最適です。",
          imageFileName1: "v5/pucks/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Violet Iridium": {
          title: "バイオレットイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Violet_Iridium_SV_puckrx",
          lightTransmission: 14,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        Shallow_Blue_Iridium_Polarized: {
          title: "シャローブルー イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 2,
          imageFileName1: "puck/Shallow_Blue_Iridium_Polarized_SV_puckrx",
          lightTransmission: 21,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_Plus_1.50_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_1.59_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ESS_1.50_Premium": {
          title: "Clear",
        },
        "Clear_OTD_ADV_Plus_1.50_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        Prizm_Rose_Gold_Polarized: {
          title: "Prizm™ ローズゴールド ポラライズド（偏光）",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Rose_Gold_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        Bronze: {
          title: "ブロンズ",
          description:
            "さまざまな厳しい条件の中で、妥協のないプロテクションを提供します。",
          imageFileName1: "v5/pucks/Bronze_SV_puckrx",
        },
        Prizm_Ruby_Polarized: {
          title: "Prizm™ ルビー ポラライズド（偏光）",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Ruby_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        "Prizm_Gaming_OTD_1.59_Premium": {
          title: "Prizm Gaming OTD 1.59 Premium",
          imageFileName1: "puck/Prizm_Gaming_SV_puckrx",
        },
        "Prizm 24K Iridium Polarized": {
          title: "Prizm‐プリズム™ 24Kイリジウム",
          imageFileName1: "v5/pucks/Prizm_24K_Polarized_SV_puckrx",
        },
        Black_Iridium: {
          title: "ブラック イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Black_Iridium_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.67_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ADV_1.50_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ESS_1.67_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "24K Iridium": {
          title: "24Kイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/24K_Iridium_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_ADV_Plus_1.67_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_1.50_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        Prizm_Deep_Water_Polarized: {
          title: "Prizm™ ディープウォーター ポラライズド（偏光）",
          description:
            "水面下が見えるように、開水域で視界を遮る青色とまぶしさをカットするように設計されています。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Deep_Water_Polarized_SV_puckrx",
          lightTransmission: 12,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Clear_OTD_ADV_Plus_1.59_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_1.50_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Black Iridium Polarized": {
          title: "ブラックイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Black_Iridium_Polarized_SV_puckrx",
          lightTransmission: 9,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm Sapphire Iridium Polarized": {
          title: "Prizm‐プリズム™ サファイアイリジウム",
          imageFileName1: "v5/pucks/Prizm_Sapphire_Polarized_SV_puckrx",
        },
        "Clear_ESS_1.50_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_ESS_1.59_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        "Ice Iridium Polarized": {
          title: "アイスイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Ice_Iridium_Polarized_SV_puckrx",
          lightTransmission: 9,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.67_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_1.59_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_ADV_Plus_1.67_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Prizm Black Polarized": {
          title: "Prizm‐プリズム™ ブラック",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Black_Polarized_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Prizm Ruby Iridium": {
          title: "Prizm‐プリズム™ ルビーイリジウム",
          imageFileName1: "v5/pucks/Prizm_Ruby_SV_puckrx",
        },
        Brown: {
          title: "ブラウン Transitions®",
          description:
            "さまざまな照明条件に対応するレンズが必要な場合に最適です。",
          imageFileName1: "puck/brown-lens",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ADV_Plus_1.50_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Positive Red Iridium": {
          title: "ポジティブ レッド イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Positive_Red_Iridium_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "バラ",
        },
        "Clear_OTD_ADV_Plus_1.50_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_1.59_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_1.59_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ESS_1.59_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Prizm Road Jade": {
          title: "Prizm™ ロード ジェード",
          description:
            "路面のわずかな変化を見つけ、石やくぼみなどの危険を素早く察知して、確実にパフォーマンスを発揮できるように設計されています。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Road_Jade_SV_puckrx",
          lightTransmission: 15,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Clear_OTD_ADV_Plus_1.67_Standard": {
          title: "Clear",
        },
        "Clear_OTD_ADV_Plus_1.67_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Prizm Sapphire Iridium": {
          title: "Prizm‐プリズム™ サファイアイリジウム",
          imageFileName1: "v5/pucks/Prizm_Sapphire_SV_puckrx",
        },
        "Prizm Rose Gold Iridium Polarized": {
          title: "Prizm‐プリズム™ ローズゴールドイリジウム",
          imageFileName1: "v5/pucks/Prizm_Rose_Gold_Polarized_SV_puckrx",
        },
        "Clear_OTD_ADV_1.59_Premium": {
          title: "Clear",
        },
        "Sapphire Transitions": {
          title: "サファイア Transitions®",
          imageFileName1: "v5/pucks/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        Prizm_Tungsten_Polarized: {
          title: "Prizm™ タングステン ポラライズド（偏光）",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          imageFileName1: "puck/Prizm_Tungsten_Polarized_SV_puckrx",
        },
        Prizm_Dark_Golf: {
          title: "Prizm™ ダーク ゴルフ",
          description:
            "フェアウェイ、フリンジ、ラフの間の変化を見分け、距離が正確に測れるように設計されているため、明るく晴れた日にコースが見極めやすくなります。",
          imageFileName1: "puck/Prizm_Dark_Golf_SV_puckrx",
        },
        "Clear_OTD_ADV_Plus_1.67_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ADV_Plus_1.59_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ESS_1.50_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ADV_1.59_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_ADV_1.67_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_ESS_1.67_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        Grey: {
          title: "グレー",
          description:
            "さまざまな厳しい条件の中で、妥協のないプロテクションを提供します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Grey_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.50_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        Prizm_Black: {
          title: "Prizm™ ブラック",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Black_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_ESS_1.59_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_1.59_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_ADV_Plus_1.59_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        Emerald_Iridium_Polarized: {
          title: "エメラルド イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Emerald_Iridium_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        Prizm_Jade: {
          title: "Prizm™ ジェード",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Jade_SV_puckrx",
          lightTransmission: 14,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_ESS_1.67_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_1.59_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Prizm Deep Water Iridium Polarized": {
          title: "Prizm‐プリズム™ ディープウォーター イリジウム",
          imageFileName1: "v5/pucks/Prizm_Deep_Water_Polarized_SV_puckrx",
        },
        Sapphire_Iridium: {
          title: "サファイア イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Sapphire_Iridium_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_Plus_1.50_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        "Prizm Tungsten Polarized": {
          title: "Prizm‐プリズム™ タングステン",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          imageFileName1: "v5/pucks/Prizm_Tungsten_Polarized_SV_puckrx",
        },
        "Clear_OTD_ESS_1.67_Premium": {
          title: "Clear",
        },
        "Prizm Ruby Polarized": {
          title: "Prizm‐プリズム™ ルビー",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Ruby_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_ADV_1.59_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        Prizm_24K: {
          title: "Prizm™ 24K",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_24K_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_ADV_1.67_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Prizm Rose Gold Polarized": {
          title: "Prizm‐プリズム™ ローズゴールド",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Rose_Gold_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        "Prizm Road Iridium": {
          title: "Prizm™ ロード イリジウム",
          description:
            "路面のわずかな変化を見つけ、石やくぼみなどの危険を素早く察知して、最高のパフォーマンスを発揮できるように設計されています。",
          imageFileName1: "v5/pucks/Prizm_Road_SV_puckrx",
        },
        "Clear_ESS_1.50_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_ADV_Plus_1.50_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_ADV_1.59_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_ADV_Plus_1.67_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        Prizm_Ruby: {
          title: "Prizm™ ルビー",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Ruby_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        Green: {
          title: "グリーン Transitions®",
          imageFileName1: "v5/pucks/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Prizm Grey": {
          title: "Prizm‐プリズム™ グレー",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Grey_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.50_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_1.59_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_ADV_1.50_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        Amber: {
          title: "アンバー Transitions®",
          imageFileName1: "v5/pucks/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        Grey_Polarized_Shield: {
          title: "グレー ポラライズド（偏光） シールド",
          description:
            "2枚の個別の度付きレンズをシールドに装着することで、視力を補正します。",
          lensClass: 2,
          imageFileName1: "puck/Grey_Polarized_Shield_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_Plus_1.59_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_ADV_1.67_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Prizm Tungsten Iridium Polarized": {
          title: "Prizm‐プリズム™ タングステンイリジウム",
          imageFileName1: "v5/pucks/Prizm_Tungsten_Polarized_SV_puckrx",
        },
        "Emerald Iridium": {
          title: "エメラルドイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Emerald_Iridium_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm Black Iridium": {
          title: "Prizm‐プリズム™ ブラック",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          imageFileName1: "v5/pucks/Prizm_Black_SV_puckrx",
        },
        Violet_Iridium: {
          title: "バイオレット イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Violet_Iridium_SV_puckrx",
          lightTransmission: 14,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm Field": {
          title: "Prizm™ フィールド",
          description:
            "コントラストを増強し、緑の芝生や茶色の土のグラウンドで野球ボールを追いやすいように設計されています。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Field_SV_puckrx",
          lightTransmission: 15,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "24K_Iridium_Polarized": {
          title: "24K イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/24K_Iridium_Polarized_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_1.67_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ADV_1.59_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_ADV_1.59_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Prizm Trail Torch Iridium": {
          title: "Prizm™ トレイル トーチ イリジウム",
          imageFileName1: "v5/pucks/Prizm_Trail_Torch_SV_puckrx",
        },
        "Clear_ESS_1.59_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        Ice_Iridium: {
          title: "アイス イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Ice_Iridium_SV_puckrx",
          lightTransmission: 10,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        Prizm_Road_Black: {
          title: "Prizm™ ロード ブラック",
          description:
            "路面のわずかな変化を見つけ、石やくぼみなどの危険を素早く察知して、確実にパフォーマンスを発揮できるように設計されています。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Road_Black_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Clear_OTD_ADV_Plus_1.50_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Amber Transitions": {
          title: "アンバー Transitions®",
          imageFileName1: "v5/pucks/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Prizm Ruby Iridium Polarized": {
          title: "Prizm‐プリズム™ ルビーイリジウム",
          imageFileName1: "v5/pucks/Prizm_Ruby_Polarized_SV_puckrx",
        },
        "Clear_OTD_1.67_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_1.59_Standard": {
          title: "Clear",
        },
        "Clear_OTD_ADV_Plus_1.59_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        Prizm_Gaming: {
          title: "Prizm™ ゲーミング",
          description:
            "シャープな視界を提供し、コントラストを増強するように設計されており、優れたブルーライトフィルター機能を搭載しています。",
          imageFileName1: "puck/Prizm_Gaming_SV_puckrx",
        },
        "Clear_OTD_1.67_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_ESS_1.59_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ADV_1.50_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_1.59_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Bronze Polarized": {
          title: "ブロンズ",
          description:
            "さまざまな厳しい条件の中で、妥協のないプロテクションを提供します。",
          lensClass: 2,
          imageFileName1: "v5/pucks/Bronze_Polarized_SV_puckrx",
          lightTransmission: 19,
          lightConditions: "ミディアムライト",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Clear_ESS_1.59_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_ESS_1.59_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_ESS_1.67_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ADV_1.59_Amber_Transition_Gen8_Standard": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_ADV_1.59_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_1.67_Premium": {
          title: "Clear",
        },
        "Clear_OTD_ADV_1.59_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_ADV_1.67_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_1.50_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Prizm Black Iridium Polarized": {
          title: "Prizm‐プリズム™ ブラックイリジウム",
          imageFileName1: "v5/pucks/Prizm_Road_Black_SV_puckrx",
        },
        "Clear_OTD_ADV_1.50_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_ESS_1.59_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Graphite Green Transitions": {
          title: "Graphite Green Transitions®",
          imageFileName1: "v5/pucks/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_ESS_1.59_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_ADV_1.59_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Prizm Gaming": {
          title: "Prizm™ ゲーミング",
          description:
            "シャープな視界を提供し、コントラストを増強するように設計されており、優れたブルーライトフィルター機能を搭載しています。",
          imageFileName1: "v5/pucks/Prizm_Gaming_SV_puckrx",
        },
        "Prizm Sapphire": {
          title: "Prizm‐プリズム™ サファイア",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Sapphire_SV_puckrx",
          lightTransmission: 12,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Prizm Trail": {
          title: "Prizm™ トレイル",
          description:
            "さまざまな照明条件のもとで、山道の変化や障害物を見つけやすいように設計されているため、自信を持ってライディングに集中できます。",
          lensClass: 2,
          imageFileName1: "v5/pucks/Prizm_Trail_SV_puckrx",
          lightTransmission: 36,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        Grey_Shield: {
          title: "グレー シールド",
          description:
            "2枚の個別の度付きレンズをシールドに装着することで、視力を補正します。",
          lensClass: 3,
          imageFileName1: "puck/Grey_Shield_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        Prizm_Rose_Gold: {
          title: "Prizm™ ローズゴールド",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Rose_Gold_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_ADV_Plus_1.59_Premium": {
          title: "Clear",
        },
        "Clear_OTD_ESS_1.67_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_1.50_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ESS_1.67_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ADV_1.67_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_ESS_1.67_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_ADV_Plus_1.50_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/RB_swatch_g15",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ADV_1.59_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        Jade_Iridium_Polarized: {
          title: "ジェード イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Jade_Iridium_Polarized_SV_puckrx",
          lightTransmission: 15,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.67_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ADV_1.59_Standard": {
          title: "Clear",
        },
        "Clear_OTD_ADV_Plus_1.67_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        Prizm_Tungsten: {
          title: "Prizm™ タングステン",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          imageFileName1: "puck/Prizm_Tungsten_SV_puckrx",
        },
        "Clear_OTD_ADV_1.59_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Prizm Low Light": {
          title: "Prizm™ Low Light",
          imageFileName1: "v5/pucks/Prizm_LowLight_SV_puckrx",
        },
        "Clear_OTD_ADV_1.67_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Prizm Deep Water Polarized": {
          title: "Prizm‐プリズム™ ディープウォーター",
          description:
            "水面下が見えるように、開水域で視界を遮る青色とまぶしさをカットするように設計されています。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Deep_Water_Polarized_SV_puckrx",
          lightTransmission: 12,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Prizm Tungsten Iridium": {
          title: "Prizm‐プリズム™ タングステンイリジウム",
          imageFileName1: "v5/pucks/Prizm_Tungsten_SV_puckrx",
        },
        "Clear_OTD_ADV_1.67_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_1.67_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        Deep_Blue_Iridium_Polarized: {
          title: "ディープブルー イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Deep_Blue_Iridium_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm Rose Gold": {
          title: "Prizm‐プリズム™ ローズゴールド",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Rose_Gold_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        Positive_Red_Iridium: {
          title: "ポジティブ レッド イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Positive_Red_Iridium_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "バラ",
        },
        "Clear_OTD_ADV_Plus_1.59_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_1.59_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_Plus_1.59_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_ADV_1.67_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_ADV_Plus_1.67_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        Ice_Iridium_Polarized: {
          title: "アイス イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Ice_Iridium_Polarized_SV_puckrx",
          lightTransmission: 9,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Prizm 24K": {
          title: "Prizm‐プリズム™ 24K",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_24K_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_ESS_1.50_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ESS_1.50_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        Bronze_Polarized: {
          title: "ブロンズ ポラライズド（偏光）",
          description:
            "さまざまな厳しい条件の中で、妥協のないプロテクションを提供します。",
          lensClass: 2,
          imageFileName1: "puck/Bronze_Polarized_SV_puckrx",
          lightTransmission: 19,
          lightConditions: "ミディアムライト",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_1.67_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_ESS_1.50_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_OTD_1.59_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_1.50_Standard": {
          title: "Clear",
        },
        Torch_Iridium: {
          title: "トーチ イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Torch_Iridium_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_Plus_1.67_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Ice Iridium": {
          title: "アイスイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Ice_Iridium_SV_puckrx",
          lightTransmission: 10,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_1.50_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        Prizm_24K_Polarized: {
          title: "Prizm™ 24K ポラライズド（偏光）",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_24K_Polarized_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "ブロンズ",
        },
        "Clear_ESS_1.59_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_1.67_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        Clear_Kids_AntiBlue: {
          title: "クリア キッズ アンチブルー",
          imageFileName1: "puck/Clear_Kids_AntiBlue_SV_puckrx",
        },
        Prizm_Road: {
          title: "Prizm™ ロード",
          description:
            "路面のわずかな変化を見つけ、石やくぼみなどの危険を素早く察知して、最高のパフォーマンスを発揮できるように設計されています。",
          lensClass: 2,
          imageFileName1: "puck/Prizm_Road_SV_puckrx",
          lightTransmission: 20,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        Prizm_Sapphire_Polarized: {
          title: "Prizm™ サファイア ポラライズド（偏光）",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Sapphire_Polarized_SV_puckrx",
          lightTransmission: 12,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.59_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_1-50": {
          title: "O オーセンティックス スリム",
          description:
            "Plutonite®は、光学的に純度が高く、耐久性のあるレンズ素材です。必要レベルを上回る衝撃プロテクションを搭載。オークリーはそれが必須だと考えています。",
        },
        "Clear_OTD_ESS_1.50_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ESS_1.50_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_ESS_1.67_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_ESS_1.67_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Grey Polarized": {
          title: "グレー",
          description:
            "さまざまな厳しい条件の中で、妥協のないプロテクションを提供します。",
          lensClass: 2,
          imageFileName1: "v5/pucks/Grey_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Deep Blue Iridium Polarized": {
          title: "ディープブルー イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Deep_Blue_Iridium_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Jade Iridium Polarized": {
          title: "ジェードイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Jade_Iridium_Polarized_SV_puckrx",
          lightTransmission: 15,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_Plus_1.59_Standard": {
          title: "Clear",
        },
        "Clear_ESS_1.67_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_ADV_1.59_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        Emerald_Iridium: {
          title: "エメラルド イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Emerald_Iridium_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_ESS_1.59_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        Prizm_Road_Jade: {
          title: "Prizm™ ロード ジェード",
          description:
            "路面のわずかな変化を見つけ、石やくぼみなどの危険を素早く察知して、確実にパフォーマンスを発揮できるように設計されています。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Road_Jade_SV_puckrx",
          lightTransmission: 15,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Clear_OTD_ADV_Plus_1.67_Premium": {
          title: "Clear",
        },
        "Clear_OTD_ADV_Plus_1.59_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Prizm Violet Polarized": {
          title: "Prizm™ Violet",
          imageFileName1: "v5/pucks/Prizm_Violet_Polarized_SV_puckrx",
        },
        "Green Transitions": {
          title: "グリーン Transitions®",
          imageFileName1: "v5/pucks/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ADV_1.67_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Tungsten Iridium": {
          title: "タングステンイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Tungsten_Iridium_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Clear_OTD_ADV_Plus_1.59_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_Plus_1.59_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_Plus_1.67_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ADV_1.50_Sapphire_Transition_Gen8_Premium": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        Black_Iridium_Polarized_Shield: {
          title: "ブラック イリジウム ポラライズド（偏光） シールド",
          description:
            "2枚の個別の度付きレンズをシールドに装着することで、視力を補正します。",
          lensClass: 3,
          imageFileName1: "puck/Black_Iridium_Polarized_Shield_SV_puckrx",
          lightTransmission: 9,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        Prizm_Jade_Polarized: {
          title: "Prizm™ ジェード ポラライズド（偏光）",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Jade_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_Plus_1.67_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Prizm Trail Iridium": {
          title: "Prizm™ トレイル イリジウム",
          imageFileName1: "v5/pucks/Prizm_Trail_SV_puckrx",
        },
        "Prizm Sapphire Polarized": {
          title: "Prizm‐プリズム™ サファイア",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Sapphire_Polarized_SV_puckrx",
          lightTransmission: 12,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Prizm Road Black Iridium": {
          title: "Prizm™ Road Black Iridium",
          imageFileName1: "v5/pucks/Prizm_Road_Black_SV_puckrx",
        },
        "Clear_1-67": {
          title: "O オーセンティックス エクストラ シン",
          description:
            "高い度数でも、より薄く、より軽量で、よりフラットなレンズを実現します。追加のコーティングなしで、紫外線を100%カットします。",
        },
        "Prizm Trail Torch": {
          title: "Prizm™ トレイル トーチ",
          description:
            "さまざまな照明条件のもとで、山道の路面の状態や変化、障害物を見つけやすいように設計されているため、自信を持ってライディングに集中できます。",
          lensClass: 2,
          imageFileName1: "v5/pucks/Prizm_Trail_Torch_SV_puckrx",
          lightTransmission: 35,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Clear_OTD_1.67_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Grey_Transitions_1-67": {
          title: "グレー Transitions® 1.67",
          description:
            "強度の処方で、さまざまな照明条件に対応するレンズが必要な場合に最適です。",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        Sapphire_Iridium_Polarized: {
          title: "サファイア イリジウム ポラライズド（偏光）",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Sapphire_Iridium_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_1.67_Standard": {
          title: "Clear",
        },
        "Clear_OTD_ADV_Plus_1.50_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ESS_1.67_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_1.59_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ADV_1.50_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_1.50_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_1.67_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_1.50_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ADV_1.67_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_ADV_Plus_1.67_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ADV_Plus_1.50_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_Plus_1.59_Amethyst_Transition_Gen8_Standard": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Clear_ESS_1.50_Emerald_Transition_Gen8_Premium": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_ESS_1.59_Premium": {
          title: "Clear",
        },
        "Prizm Road": {
          title: "Prizm™ ロード",
          description:
            "路面のわずかな変化を見つけ、石やくぼみなどの危険を素早く察知して、最高のパフォーマンスを発揮できるように設計されています。",
          lensClass: 2,
          imageFileName1: "v5/pucks/Prizm_Road_SV_puckrx",
          lightTransmission: 20,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Clear_OTD_ESS_1.67_Standard": {
          title: "Clear",
        },
        "Sapphire Iridium Polarized": {
          title: "サファイアイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Sapphire_Iridium_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        Clear: {
          title: "Plutonite™ プレミアム シン",
          description:
            "さまざまな照明条件に対応するレンズが必要な場合に最適です。",
        },
        "Clear_1-74": {
          title: "O オーセンティックス 1.74",
          description:
            "オークリーのレンズの中でも最も薄く軽量で、高い度数に適したレンズです。追加のコーティングなしで、紫外線を100%カットします。",
        },
        "Clear_OTD_ADV_Plus_1.67_Bluelight_Standard": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_Plus_1.59_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Prizm Rose Gold Iridium": {
          title: "Prizm‐プリズム™ ローズゴールドイリジウム",
          imageFileName1: "v5/pucks/Prizm_Rose_Gold_SV_puckrx",
        },
        Emerald: {
          title: "エメラルド Transitions®",
          imageFileName1: "v5/pucks/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_1.50_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Ruby Iridium Polarized": {
          title: "ルビーイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Ruby_Iridium_Polarized_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_1.50_Amber_Transition_Gen8_Premium": {
          title: "Amber Transitions®",
          imageFileName1: "puck/amber-lens",
          imageFileName2: "puck/amber-lens",
        },
        "Clear_OTD_ADV_Plus_1.67_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "24K Iridium Polarized": {
          title: "24Kイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/24K_Iridium_Polarized_SV_puckrx",
          lightTransmission: 11,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        "Ruby Iridium": {
          title: "ルビーイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Ruby_Iridium_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_1.67_Emerald_Transition_Gen8_Standard": {
          title: "Emerald Transitions®",
          imageFileName1: "puck/emerald-lens",
          imageFileName2: "puck/emerald-lens",
        },
        "Clear_OTD_1.59_Brown_Transition_Gen8_Standard": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        Prizm_Trail: {
          title: "Prizm™ トレイル",
          description:
            "さまざまな照明条件のもとで、山道の変化や障害物を見つけやすいように設計されているため、自信を持ってライディングに集中できます。",
          lensClass: 2,
          imageFileName1: "puck/Prizm_Trail_SV_puckrx",
          lightTransmission: 36,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Jade Iridium": {
          title: "ジェードイリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Jade_Iridium_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "グレー",
        },
        "Clear_OTD_1.50_Premium": {
          title: "Clear",
        },
        Prizm_Field: {
          title: "Prizm™ フィールド",
          description:
            "コントラストを増強し、緑の芝生や茶色の土のグラウンドで野球ボールを追いやすいように設計されています。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Field_SV_puckrx",
          lightTransmission: 15,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Clear_OTD_ADV_Plus_1.50_Premium": {
          title: "Clear",
        },
        "Clear_OTD_1.50_Bluelight_Premium": {
          title: "Clear with Blue Ready",
        },
        "Clear_OTD_ADV_1.59_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_ADV_Plus_1.67_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Prizm Dark Golf": {
          title: "Prizm™ ダーク ゴルフ",
          description:
            "フェアウェイ、フリンジ、ラフの間の変化を見分け、距離が正確に測れるように設計されているため、明るく晴れた日にコースが見極めやすくなります。",
          imageFileName1: "v5/pucks/Prizm_Dark_Golf_SV_puckrx",
        },
        "Clear_OTD_ADV_Plus_1.59_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        "Clear_OTD_ADV_1.67_Premium": {
          title: "Clear",
        },
        "Prizm Jade Polarized": {
          title: "Prizm‐プリズム™ ジェード",
          description:
            "色やコントラストを増強するPrizm™ レンズテクノロジーに、オークリーHDPolarizedのまぶしさを抑えるフィルター効果をプラスしました。",
          lensClass: 3,
          imageFileName1: "v5/pucks/Prizm_Jade_Polarized_SV_puckrx",
          lightTransmission: 13,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        Prizm_Trail_Torch: {
          title: "Prizm™ トレイル トーチ",
          description:
            "さまざまな照明条件のもとで、山道の路面の状態や変化、障害物を見つけやすいように設計されているため、自信を持ってライディングに集中できます。",
          lensClass: 2,
          imageFileName1: "puck/Prizm_Trail_Torch_SV_puckrx",
          lightTransmission: 35,
          lightConditions: "ミディアムライト",
          contrast: "増加しました",
          baseLensColor: "バラ",
        },
        "Clear_OTD_ESS_1.59_Green_Transition_Gen8_Standard": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ESS_1.59_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_1.50_Green_Transition_Gen8_Premium": {
          title: "Green Transitions®",
          imageFileName1: "puck/green-g-15-lens",
          imageFileName2: "puck/green-g-15-lens",
        },
        "Clear_OTD_ADV_1.67_Grey_Transition_Gen8_Standard": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Clear_OTD_ESS_1.59_Brown_Transition_Gen8_Premium": {
          title: "Brown Transitions®",
          imageFileName1: "puck/Brown_Transitions_SV_puckrx",
          imageFileName2: "puck/brown-lens",
        },
        Prizm_Grey: {
          title: "Prizm™ グレー",
          description:
            "くすんだ色を生き生きとさせ、周囲の最も鮮やかで明るい部分を強調します。",
          lensClass: 3,
          imageFileName1: "puck/Prizm_Grey_SV_puckrx",
          lightTransmission: 17,
          lightConditions: "明るい光",
          contrast: "増加しました",
          baseLensColor: "グレー",
        },
        "Clear_OTD_ADV_1.67_Amethyst_Transition_Gen8_Premium": {
          title: "Amethyst Transitions®",
          imageFileName1: "puck/amethyst-lens",
          imageFileName2: "puck/amethyst-lens",
        },
        "Prizm Dark Golf Iridium": {
          title: "Prizm™ Dark Golf Iridium",
          imageFileName1: "v5/pucks/Prizm_Dark_Golf_SV_puckrx",
        },
        "Clear_OTD_1.50_Sapphire_Transition_Gen8_Standard": {
          title: "Sapphire Transitions®",
          imageFileName1: "puck/sapphire-lens",
          imageFileName2: "puck/sapphire-lens",
        },
        "Clear_OTD_ADV_Plus_1.50_Grey_Transition_Gen8_Premium": {
          title: "Grey Transitions®",
          imageFileName1: "puck/Grey_Transitions_SV_puckrx",
          imageFileName2: "puck/grey-lens",
        },
        "Prizm Violet": {
          title: "Prizm™ Violet",
          imageFileName1: "v5/pucks/Prizm_Violet_SV_puckrx",
        },
        Fire_Iridium: {
          title: "ファイヤー イリジウム",
          description:
            "イリジウムレンズコーティングは、特定の色を反射させるとともに、レンズのベースの色調と連携して、バランスのとれた光透過率を実現します。",
          lensClass: 3,
          imageFileName1: "puck/Fire_Iridium_SV_puckrx",
          lightTransmission: 16,
          lightConditions: "明るい光",
          contrast: "中性",
          baseLensColor: "ブロンズ",
        },
        Clear_Shield: {
          title: "クリア シールド",
          description:
            "2枚の個別の度付きレンズをシールドに装着することで、視力を補正します。",
          lensClass: 0,
          imageFileName1: "puck/Clear_Shield_SV_puckrx",
          lightTransmission: 93,
          lightConditions: "低光",
          contrast: "中性",
          baseLensColor: "クリア",
        },
        "Clear_OTD_1.67_Standard": {
          title: "Clear",
        },
      },
      transition: {
        Transitions: {
          imageFileName1:
            "rxc-icon-transitions-transitions®_signature®_gen8™_oo",
          imageFileName2:
            "rxc-icon-3d-transitions-transitions®_signature®_gen8™_oo",
          title: "Transitions® Signature® Gen 8™",
          description:
            "Transitions®レンズは、太陽光を浴びると色が素早くダークに変化し、屋内ではクリアに戻ります。カラーバリエーションも豊富。ブルーバイオレットライトフィルターを搭載しています。 400～500nmのLEDブルーライトを30%削減。",
        },
        Signature: {
          imageFileName1:
            "rxc-icon-transitions-transitions®_signature®_gen8™_oo",
          imageFileName2:
            "rxc-icon-3d-transitions-transitions®_signature®_gen8™_oo",
          title: "Transitions® Signature® Gen 8™",
          description:
            "Transitions®レンズは、太陽光を浴びると色が素早くダークに変化し、屋内ではクリアに戻ります。カラーバリエーションも豊富。ブルーバイオレットライトフィルターを搭載しています。 400～500nmのLEDブルーライトを30%削減。",
        },
        Xtractive: {
          imageFileName1:
            "rxc-icon-transitions-transitions®_xtractive®_icon_oo",
          imageFileName2:
            "rxc-icon-3d-transitions-transitions®_xtractive®_icon_oo",
          title: "Transitions® XTRActive® New Generation",
          description:
            "特に光に敏感な方や強い光にさらされる機会が頻繁にある方、また車を運転する時にも暗めのレンズカラーをご希望の方にぴったりな、クリアからダークへと色が変化するレンズです。ブルーバイオレットライトフィルターを搭載しています。 400～500nmのLEDブルーライトを30%削減。",
        },
        "Transition Xtractive": {
          imageFileName1:
            "rxc-icon-transitions-transitions®_xtractive®_icon_oo",
          imageFileName2:
            "rxc-icon-3d-transitions-transitions®_xtractive®_icon_oo",
          title: "Transitions® XTRActive® ニュージェネレーション",
          description:
            "特に光に敏感な方や強い光にさらされる機会が頻繁にある方、また車を運転する時にも暗めのレンズカラーをご希望の方にぴったりな、クリアからダークへと色が変化するレンズです。ブルーバイオレットライトフィルターを搭載しています。 400～500nmのLEDブルーライトを30%削減。",
        },
      },
      thickness: {
        UltraThin: {
          imageFileName1: "icon/index-1_67-icon",
          title: "O Authentics 1.67",
          sequence: 1,
          description:
            "A lighter lens with superior clarity and resistance to breakage.",
        },
        "Premium UltraThin": {
          imageFileName1: "v5/icons/index-1_67-icon",
          title: "O Authentics 1.74",
          sequence: 3,
          description:
            "An even lighter and discreet lens ideal for strong prescriptions.",
        },
        "Extra thin": {
          imageFileName1: "rxc-icon-thickness-superior_extra_thin_lenses_clear",
          imageFileName2:
            "rxc-icon-3d-thickness-superior_extra_thin_lenses_clear",
          title: "O Authentics 1.67 Extra Thin",
          sequence: 3,
          description:
            "さらに薄く、軽く、フラットで、100%のUVプロテクション効果も発揮する度付きレンズです。（度数範囲：+4.00以上または-4.00以下).",
        },
        "Premium Thin": {
          imageFileName1: "rxc-icon-thickness-superior_extra_thin_lenses_clear",
          imageFileName2:
            "rxc-icon-3d-thickness-superior_extra_thin_lenses_clear",
          title: "O オーセンティックス 1.67 エクストラ シン",
          sequence: 3,
          description:
            "さらに薄く、軽く、フラットで、100%のUVプロテクション効果も発揮する度付きレンズです。（度数範囲：+4.00以上または-4.00以下).",
        },
        Standard: {
          imageFileName1: "rxc-icon-thickness-enhanced_slim_lenses_clear",
          imageFileName2: "rxc-icon-3d-thickness-enhanced_slim_lenses_clear",
          title: "O オーセンティックス 1.50 スリム",
          sequence: 1,
          description:
            "ガラスレンズの約半分の重さで飛散しにくく、100%のUVプロテクション効果を発揮するレンズです。（度数範囲：+2.00～-2.00).",
        },
        Slim: {
          imageFileName1: "rxc-icon-thickness-enhanced_slim_lenses_clear",
          imageFileName2: "rxc-icon-3d-thickness-enhanced_slim_lenses_clear",
          title: "O Authentics 1.50 Slim",
          sequence: 1,
          description:
            "ガラスレンズの約半分の重さで飛散しにくく、100%のUVプロテクション効果を発揮するレンズです。（度数範囲：+2.00～-2.00).",
        },
        Thin: {
          imageFileName1: "rxc-icon-thickness-premium_thin_lenses_clear",
          imageFileName2: "rxc-icon-3d-thickness-premium_thin_lenses_clear",
          title: "Plutonite® 1.59 シン",
          sequence: 2,
          description:
            "スポーツに理想的なPlutonite®レンズは、妥協のない保護性能と耐久性を持ち、コーティングではなくレンズ自体がUVプロテクション効果を発揮します。（度数範囲：+4.00～-4.00).",
        },
        "Standard Thin": {
          imageFileName1: "rxc-icon-thickness-premium_thin_lenses_clear",
          imageFileName2: "rxc-icon-3d-thickness-premium_thin_lenses_clear",
          title: "Plutonite® 1.59 シン",
          sequence: 2,
          description:
            "スポーツに理想的なPlutonite®レンズは、妥協のない保護性能と耐久性を持ち、コーティングではなくレンズ自体がUVプロテクション効果を発揮します。（度数範囲：+4.00～-4.00).",
        },
      },
      brand: {
        Oakley: {
          imageFileName1: "rxc-icon-brands-oo_authentic_clear",
          imageFileName2: "rxc-icon-3d-brands-oakley_clear",
          title: "Oakley",
          description: "Oakley",
        },
      },
      antiReflective: {
        "Premium Anti reflective": {
          imageFileName1:
            "rxc-icon-finishing-anti_reflective_anti_smudge_clear_oo",
          imageFileName2:
            "rxc-icon-3d-finishing-anti_reflective_anti_smudge_clear_oo",
          title: "オークリー Stealth™ プロ",
          description:
            "反射防止加工は、紫外線やキズに対する保護効果を発揮し、汚れや油分、水分、ほこりをはじいてレンズを清潔に保つのにも役立ちます。",
        },
        Premium: {
          imageFileName1:
            "rxc-icon-finishing-anti_reflective_anti_smudge_clear_oo",
          imageFileName2:
            "rxc-icon-3d-finishing-anti_reflective_anti_smudge_clear_oo",
          title: "Oakley Stealth™ Pro",
          description:
            "反射防止加工は、紫外線やキズに対する保護効果を発揮し、汚れや油分、水分、ほこりをはじいてレンズを清潔に保つのにも役立ちます。",
        },
        "Standard Anti reflective": {
          imageFileName1: "rxc-icon-finishing-anti_reflective_clear_oo",
          imageFileName2: "rxc-icon-3d-finishing-anti_reflective_clear_oo",
          title: "反射防止加工",
          description:
            "レンズを清潔に保ち、不要な反射をなくすように設計されています。",
        },
      },
      type: {
        SINGLE_VISION: {
          imageFileName1: "rxc-icon-types-single_vision_dots_sun_oo",
          imageFileName2: "rxc-icon-3d-types-single_vision_dots_sun_oo",
          title: "単焦点レンズ",
          sequence: 1,
          description: "近視、中間視力、遠視を補正します。",
        },
        Frame_only: {
          imageFileName1: "rxc-icon-types-frame_only_clear_rb",
          imageFileName2: "rxc-icon-3d-types-frame_only_clear_rb",
          title: "フレームのみ",
          sequence: 4,
          description: "軽量で高性能なフレーム。レンズや加工の追加なし。",
        },
        Non_prescription: {
          imageFileName1: "rxc-icon-types-nonprescription_lenses_oo",
          imageFileName2: "rxc-icon-3d-types-nonprescription_lenses_oo",
          title: "Non-prescription",
          sequence: 3,
          description:
            "視力矯正を含まない軽量でパフォーマンス性能が高いフレームに、ブルーバイオレットライト*のフィルター仕様と、Transitions-トランジション®レンズのオプションを追加できます。",
        },
        PROGRESSIVE: {
          imageFileName1:
            "rxc-icon-types-progressive_multifocal_sun_comfort_oo_no_icon",
          imageFileName2:
            "rxc-icon-3d-types-progressive_multifocal_sun_comfort_oo_no_icon",
          title: "累進レンズ",
          sequence: 2,
          description:
            "1つのレンズで近視、中間視力、遠視を補正するため、メガネを掛け替える必要がありません。",
        },
      },
      designType: {
        OakleyTrueDefinition: {
          imageFileName1: "rxc-icon-designs-single_vision_true_digital_oo",
          imageFileName2: "rxc-icon-3d-designs-single_vision_true_digital_oo",
          title: "オークリー True Digital™",
          sequence: 5,
          description:
            "OTD™レンズは、レンズの裏面に複雑なデジタル処理を行うレンズテクノロジーで、レンズの表面全体がより鮮明に最適化されて見えるように設計されています。",
        },
        "OTD™ Advance": {
          imageFileName1: "rxc-icon-designs-otd_advance_oo",
          imageFileName2: "rxc-icon-3d-designs-otd_advance_oo",
          title: "オークリー True Digital™ アドバンス",
          description:
            "Oakley True Digital™は、デジタル機器が欠かせない現代のライフスタイルのために強化されたレンズテクノロジーです。OTD™ Advanceは、独自のフレームデータベースを使用して、ユーザーとフレームに合ったレンズをカスタム設計し、視覚的な鮮明さを最適化。違和感なく画面を見ることができます。",
        },
        PrizmEveryday: {
          title: "Prizm™ エブリデイ",
          description:
            "周囲がより鮮やかに、細かい部分までくっきりと見えるように設計されているため、体験の充実感が高まります。",
        },
        PrizmSport: {
          title: "Prizm™ スポーツ",
          description:
            "スポーツに欠かせないツールとして特別に設計されているため、路上、競技場、コース、トレイル、水上でワンランク上のパフォーマンスを提供します。",
        },
        "OTD™ Advance Plus": {
          imageFileName1: "rxc-icon-designs-otd_advance_plus_oo_no_icon",
          imageFileName2: "rxc-icon-3d-designs-otd_advance_plus_oo_no_icon",
          title: "オークリー True Digital™ アドバンス プラス",
          description:
            "OTD™ Advanceは、視力矯正の種類ごとに異なるレンズ設計を取り入れることで、ユーザーがレンズにスムーズに適応できるように最適化されたレンズテクノロジーです。",
        },
        Essential: {
          imageFileName1: "rxc-icon-designs-oo_essential_plus",
          imageFileName2: "rxc-icon-3d-designs-oo_essential_plus",
          title: "エッセンシャル プラス",
          sequence: 4,
          description:
            "全てのオークリー純正度付きレンズにはレーザーエッチングによるオークリーロゴが入っています。イノベーションによる既存の枠を超えたデザインと業界をリードする最新のテクノロジーを備えたレンズは、世界中のアスリートも使用しています。",
        },
        OakleyEssential: {
          imageFileName1: "icon/essential-icon",
          title: "オークリー エッセンシャル",
          sequence: 3,
          description:
            "日常のニーズに応えるオークリーの革新的なデザインDNAです。",
        },
        OakleyStandard: {
          title: "スタンダード",
          description:
            "オークリーのサングラスレンズは、バラエティに富んだカラーが揃っています。",
        },
        PrizmGaming: {
          imageFileName1: "rxc-icon-treatment_family-prizm™_gaming_logo_oo",
          imageFileName2: "rxc-icon-3d-treatment_family-prizm™_gaming_logo_oo",
          title: "Prizm™ ゲーミング",
          description:
            "Prizm Gaming™ 2.0レンズは色とコントラストを増強し、有機ELディスプレイやLEDスクリーンからのブルーライトをカットするように設計されています。 400～500nmのLEDブルーライトを30%削減。",
        },
      },
      colorCategory: {
        PRIZM_SPORT: {
          title: "Prizm™ スポーツ",
          description:
            "それぞれのスポーツ環境に合わせて、細部まで的確に見えるようにデザインされています。",
          uxPanelType: "RECOMMENDED",
        },
        POLARIZED: {
          title: "ポラライズド（偏光）",
          description:
            "オークリーのHDPolarizedは、まぶしさをブロックして目の疲れを軽減するように設計されています。",
        },
        PRIZM: {
          title: "Prizm™",
          description:
            "日常的なシーンで色とコントラストを増強し、細部までくっきりと見えるようになります。",
        },
        PRIZM_POLARIZED: {
          title: "Prizm™ ポラライズド（偏光）",
          description:
            "まぶしさを抑えて目の疲れを最小限にし、日常的なシーンで色とコントラストを増強するようにデザインされています。",
        },
        STANDARD: {
          title: "スタンダード",
          description:
            "オークリーのサングラスレンズは日常生活に最適で、バラエティに富んだカラーが揃っています。",
        },
      },
      polar: {
        "polar-true": {
          title: "ポラライズド（偏光）",
          description:
            "オークリーのHDPolarizedは、まぶしさを99%ブロックして目の疲れを軽減するように設計されています。",
        },
      },
      treatmentFamily: {
        TransitionsFamily: {
          imageFileName1:
            "rxc-icon-transitions-transitions®_signature®_gen8™_oo",
          imageFileName2:
            "rxc-icon-3d-transitions-transitions®_signature®_gen8™_oo",
          title: "Transitions® - 調光タイプ",
          description:
            "光のコンディションに合わせて、屋外では濃い色に、屋内では透明に変化するレンズ。ブルーバイオレットライトフィルターを搭載しています。 400～500nmのLEDブルーライトを30%削減。",
        },
      },
      promo: {
        promo40off_v3: {
          title: "Your 40% off (including lenses) is automatically applied",
        },
        promo50off: {
          title: "50% off Prescription Lenses is automatically applied",
        },
        promo40off: {
          title: "40% off Prescription Lenses is automatically applied",
        },
        promo50eyestealthoproff: {
          title: "up to 50% off complete pair + Stealth Pro for free at cart.",
        },
        promo50offOSPfree: {
          title: "50% off lenses + Oakley Stealth Pro for free at checkout",
        },
        promo50sunoff: {
          title: "up to 50% off your prescription complete pair.",
        },
        promo60usdoff: {
          title: "Add to bag to score $60 off your prescription glasses.",
        },
        promo40off_v2: {
          title:
            "40% off Oakley Authentic Prescription (including lenses) is automatically applied",
        },
      },
    },
  },
  paymentInstallment: {
    type: "affirm",
    installments: 3,
  },
  /* OPTIONAL insurance module, if not provided, insurance buttons won't be visible */
  insuranceModule: {
    _isEnabled: false /* DEV use, do not use or implement */,
    _isLoading: false /* DEV use, do not use or implement */,
    _isLogged: false /* DEV use, do not use or implement */,
    isEnabled: function () {
      return this._isEnabled;
    },
    isLoading: function () {
      return this._isLoading;
    },
    isLogged: function () {
      return this._isLogged;
    },
    getEvents: function () {
      return {
        INSURANCE_LOGIN: "insuranceLogin",
        INSURANCE_LOGOUT: "insuranceLogout",
        INSURANCE_ENABLED: "insuranceEnabled",
        INSURANCE_DISABLED: "insuranceDisabled",
      };
    },
    getInsuranceDiscounts: function (_selectedFrame, _selectedPackages) {
      //alert('Calling insurance module getDiscounts()');

      return new Promise(function (resolve, reject) {
        // Sample 30% insurance discount
        var _insurancePackages = JSON.parse(JSON.stringify(_selectedPackages));

        for (let i = 0; i < _insurancePackages.length; i++) {
          var lensListPrice = parseFloat(
            _insurancePackages[i].lensPackage.listPrice
          );
          _insurancePackages[i].lensPackage.insPrice = parseFloat(
            (lensListPrice - lensListPrice * 0.3).toFixed(2)
          );
        }

        setTimeout(function () {
          resolve(_insurancePackages);
        }, 2000);
      });
    },
    openInsurancePanel: function () {
      //if (window.confirm('Calling insurance module openInsurancePanel()')) {
      this._isLogged = true;
      this._isEnabled = true;
      window.dispatchEvent(new CustomEvent(this.getEvents().INSURANCE_ENABLED));
      //} else {
      //do nothing
      //}
    },
    removeInsuranceBenefits: function () {
      //if (window.confirm('Calling insurance module removeInsuranceBenefits()')) {
      this._isLogged = true;
      this._isEnabled = false;
      window.dispatchEvent(
        new CustomEvent(this.getEvents().INSURANCE_DISABLED)
      );
      //} else {
      //do nothing
      //}
    },
  },
  prescriptionModule: {
    //nestedFlows:["UPLOAD","CMD"],
    prescriptionType: "FULL", // or FULL, case of Glasses.com after lens type selection
    prescriptionFlows: ["MANUAL", "CMD", "UPLOAD"], //'MANUAL','ACCOUNT','UPLOAD','CMD'
    fileExtensions: [
      "png",
      "gif",
      "jpeg",
      "tiff",
      "bmp",
      "word",
      "pdf",
      "pages",
    ],
    maxFileSize: "10",
    /* abTest: {
            prescriptionManualOrder: 4,
            prescriptionFromMyAccountOrder: 1,
            prescriptionUploadOrder: 2,
            prescriptionDoctorOrder: 3,
            prescriptionSendLaterOrder: 5
        }, */
    loadPrescription: function () {
      var _prescriptionObject = null;
      return new Promise(function (resolve, reject) {
        var value = null;
        var name = "_prescriptionObject=";
        var decodedCookie = decodeURIComponent(document.cookie);
        var ca = decodedCookie.split(";");
        for (var i = 0; i < ca.length; i++) {
          var c = ca[i];
          while (c.charAt(0) == " ") {
            c = c.substring(1);
          }
          if (c.indexOf(name) == 0) {
            value = c.substring(name.length, c.length);
          }
        }

        if (value) {
          try {
            _prescriptionObject = JSON.parse(value);

            alert(
              "Calling loadPrescription()\n\nPrescription object: " +
                JSON.stringify(_prescriptionObject)
            );
            // validation is run by the configuration agains the return object
            resolve(_prescriptionObject);
          } catch (e) {
            console.log(e);
            reject();
          }
        } else {
          resolve(null);
        }
      });
    },

    loadExtendedPrescription: function () {
      var _prescriptionObject = null;
      return new Promise(function (resolve, reject) {
        var value = null;
        var name = "_prescriptionObject=";
        var decodedCookie = decodeURIComponent(document.cookie);
        var ca = decodedCookie.split(";");
        for (var i = 0; i < ca.length; i++) {
          var c = ca[i];
          while (c.charAt(0) == " ") {
            c = c.substring(1);
          }
          if (c.indexOf(name) == 0) {
            value = c.substring(name.length, c.length);
          }
        }

        if (value) {
          try {
            _prescriptionObject = JSON.parse(value);

            /* alert(
                            'Calling loadPrescription()\n\nPrescription object: ' +
                                JSON.stringify(_prescriptionObject)
                        ); */
            // validation is run by the configuration agains the return object
            resolve(_prescriptionObject);
          } catch (e) {
            console.log(e);
            reject();
          }
        } else {
          let test = {
            prescriptionFlow: "MANUAL", // Value is one of the prescriptionFlow flags
            prescriptionId: 12345, // prescriptionId is provided by the e-commerce
            CYL: {
              OS: "-0.5",
              OD: "0.25",
            },
            AX: {
              OS: "3.0",
              OD: "2.0",
            },
            SPH: {
              OS: "0.5",
              OD: "0.5",
            },
            PD: 0,
            ADD: {
              OS: "0",
              OD: "0",
            },
          };
          resolve(null);
        }
      });
    },

    savePrescription: function (_prescriptionObject) {
      alert(
        "Calling savePrescription()\n\nPrescription object: " +
          JSON.stringify(_prescriptionObject)
      );
      //sample cookie saving
      document.cookie =
        "_prescriptionObject=" + JSON.stringify(_prescriptionObject);
      return true; // or false if failure
    },

    saveExtendedPrescription: function (_prescriptionObject) {
      alert(
        "Calling saveExtendedPrescription()\n\nPrescription object: " +
          JSON.stringify(_prescriptionObject)
      );
      //sample cookie saving
      document.cookie =
        "_prescriptionObject=" + JSON.stringify(_prescriptionObject);
      return new Promise(function (resolve, reject) {
        let newId = 12345;
        let ret = JSON.parse(JSON.stringify(_prescriptionObject));
        ret["prescriptionId"] = newId;
        resolve(ret);
      });
    },

    clearExtendedPrescription: function (_requestObj) {
      document.cookie =
        "_prescriptionObject" + "=;expires=Thu, 01 Jan 1970 00:00:01 GMT;";
      return new Promise(function (resolve, reject) {
        resolve({});
      });
    },

    uploadExtendedPrescription: function (_requestObject) {
      return new Promise(function (resolve, reject) {
        setTimeout(() => {
          let ret = {
            prescriptionId: 1234555,
            fileName: _requestObject.fileName,
            /* filePath: 'https://www.researchgate.net/profile/Ranju-Mandal/publication/286725383/figure/fig1/AS:685206105243649@1540377274394/A-sample-printed-signed-document-from-Tobacco-800-dataset-A-zoomed-version-of-the.png' */
            savedFileName: "document.png",
          };
          resolve(ret);
        }, 2000);
      });
    },

    downloadExtendedPrescription: function (_requestObject) {
      return new Promise(function (resolve, reject) {
        setTimeout(() => {
          let ret = {
            fileData:
              "data:image/png;base64,",
          };
          resolve(ret);
        }, 2000);
      });
    },

    searchDoctors: function (_requestObject) {
      return new Promise(function (resolve, reject) {
        let ret = {
          doctors: [
            {
              name: "Pinco pallino",
              clinic: "Clinic pallino",
              address: "Via pinco pallino n.1",
              phone: "+391231458930",
              fax: "+3900008219",
            },
            {
              name: "Pinco pallino 2",
              clinic: "Clinic pallino",
              address: "Via pinco pallino n.1",
              phone: "+391231458930",
              fax: "+3900008219",
            },
            {
              name: "Pinco pallino 3",
              clinic: "Clinic pallino",
              address: "Via pinco pallino n.1",
              phone: "+391231458930",
              fax: "+3900008219",
            },
            {
              name: "Pinco pallino 4",
              clinic: "Clinic pallino",
              address: "Via pinco pallino n.1",
              phone: "+391231458930",
              fax: "+3900008219",
            },
          ],
        };
        setTimeout(() => {
          resolve(ret);
        }, 2000);
      });
    },

    // if no lenses are available against the prescription filtering, this function is called to retrieve the frames filtered PLP URL to link in the dedicated CTA "see available frames"
    // the PLP must be of the same category of the frame
    // if no URL is returned, then the user won't see the CTA
    checkAvailableFrames: function (_frame, _prescriptionObject) {
      alert(
        "Calling checkAvailableFrames().\n\nFrame object: " +
          JSON.stringify(_frame) +
          "\n\nPrescription object: " +
          JSON.stringify(_prescriptionObject)
      );
      //var filteredFramesURL = "/lc-us/sunglasses";
      var filteredFramesURL = null;
      return filteredFramesURL;
    },

    // it removes the cookie information
    clearPrescription: function () {
      document.cookie = "_prescriptionObject=; Max-Age=-99999999;";
    },
  },
  selector: "#rxcApp",
  translation: {
    language: "it_CH",
    brand: "OO",
  },
  unbrandedFrameImages: {
    rayban: {
      default:
        "https://assets.glasses.com/extra/image/Glasses/#PARTNUMBER#__002.png?impolicy=GL_parameters_transp&width=860",
      unbranded:
        "https://assets.glasses.com/extra/image/rxc/frames/#MO#__#CO#__RXP__nologo__qt.png?impolicy=GL_parameters_transp&width=860",
      rayban:
        "https://assets.glasses.com/extra/image/Glasses/#PARTNUMBER#__002.png?impolicy=GL_parameters_transp&width=860",
    },
    raybanjr: {
      default:
        "https://assets.glasses.com/extra/image/Glasses/#PARTNUMBER#__002.png?impolicy=GL_parameters_transp&width=860",
      unbranded:
        "https://assets.glasses.com/extra/image/rxc/frames/#MO#__#CO#__RXP__nologo__qt.png?impolicy=GL_parameters_transp&width=860",
      rayban:
        "https://assets.glasses.com/extra/image/Glasses/#PARTNUMBER#__002.png?impolicy=GL_parameters_transp&width=860",
    },
    oakley: {
      default:
        "https://assets.glasses.com/extra/image/Glasses/#PARTNUMBER#__002.png?impolicy=GL_parameters_transp&width=860",
      unbranded:
        "https://assets.glasses.com/extra/image/Glasses/#PARTNUMBER#__002.png?impolicy=GL_parameters_transp&width=860",
      oakley:
        "https://assets.glasses.com/extra/image/rxc/frames/#MO#__#CO#__RXP__logo__qt.png?impolicy=GL_parameters_transp&width=860",
    },
    default: {
      default:
        "https://assets.glasses.com/extra/image/Glasses/#PARTNUMBER#__002.png?impolicy=GL_parameters_transp&width=860",
      unbranded:
        "https://assets.glasses.com/extra/image/Glasses/#PARTNUMBER#__002.png?impolicy=GL_parameters_transp&width=860",
    },
  },
  dynamicPromoModule: {
    getDiscount: function (selectedFrame, selectedData) {
      console.log("Simulating getDiscount");
      return new Promise(function (resolve, reject) {
        const framePrice = selectedFrame.listPrice;

        const lensePackagePrice = parseFloat(
          selectedData.lensPackage.listPrice
        );
        const lenseFramePrice = parseFloat(selectedData.frame.offerPrice);

        setTimeout(function () {
          resolve({
            lensesData: {
              lensPackage: {
                ...selectedData.lensPackage,
                offerPrice: parseFloat((lensePackagePrice * 0.4).toFixed(2)),
              },
              frame: {
                ...selectedData.frame,
                offerPrice: parseFloat((lenseFramePrice * 0.4).toFixed(2)),
              },
            },
            frame: {
              ...selectedFrame,
              offerPrice: parseFloat((framePrice * 0.4).toFixed(2)),
            },
          });
        }, Math.floor(Math.random() * 5000));
      });
    },
  },
};
